...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        for l in range(1, args.num_linear_layers-1):
            setattr(self, 'fc{}'.format(l), torch.nn.Linear(args.emb_dim, int(int(args.emb_dim)/2)))
        setattr(self, 'fc{}'.format(args.num_linear_layers-1), torch.nn.Linear(max(4,int(int(args.emb_dim)/(2^(args.max_k-2)))), dataset.num_classes))
